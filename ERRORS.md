...
| CAPACITY_EXCEEDED | In-memory reference limit reached | Use the separate persistence/export silo where appropriate or start a separate workspace; do not drop history |
...
Reserved categories, **not outcomes implemented by the core**: REVIEW_REQUIRED, AMBIGUOUS, INSUFFICIENT_EVIDENCE, AUTHORITY_CONFLICT, UNSUPPORTED, CAPABILITY_UNAVAILABLE, DEPENDENCY_UNAVAILABLE, LOSSY_OPERATION, LOSSY_EXPORT, ROLLBACK_FAILED, SECURITY_BLOCKED, RIGHTS_RESTRICTED, INTERNAL_FAILURE. Some names appear in separate silo contracts. Uncertainty in the core belongs to proposals, not mutation failures. Core success returns a Revision; partial success is unsupported.
...
## Evaluation errors — experimental v0.1.01 reference
